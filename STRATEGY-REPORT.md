...
| 版本 | 日期 | 核心变更 | 最佳 Alpha |
|------|------|---------|-----------|
| **V1** | 2026-03-29 | 纯技术策略 + 单标的价值策略 | -13.7% |
| **V2** | 2026-03-29 | 组合策略重构（多因子选股 + 动量 + Regime Filter + Trailing Stop） | -1.5% |
| **V3** | 2026-03-29 | S&P 100 标的池 + 历史财报修复前视偏差 | +3.3% |
| **V3.1** | 2026-03-29 | S&P 500 核心 ~150 只 + Earnings Surprise 因子 | -0.1% |
| **V4** | 2026-03-29 | 跨资产 Regime Filter（收益率曲线 + VIX + 信用利差） | +3.9% |
| **V5** | 2026-03-29 | Short Interest 过滤 + Insider Trading 信号 | +4.0% |
| **V6** | 2026-03-29 | 历史新闻情绪因子（HuggingFace 数据集） | **+4.0%** |
...
*V2 报告完整数据见 `reports/portfolio_validation_*.txt` 和 `.json`。*
...
从 2023-2025（3年牛市）扩展到 2018-2025（7年），覆盖：
- 2018 年末贸易战回调
- 2020 年新冠崩盘（-34% 后 V 型反弹）
- 2022 年加息熊市（-25%）
- 2023-2025 年 AI 牛市
...
|------|--------|------|-------|------|---------|------|---------|----------|
| **V3 Standard (Historical)** | +103.2% | +26.8% | **+3.3%** | **1.41** | -13.7% | 66% | 9.2 | **Y** |
| V3 Aggressive (Historical) | +98.8% | +25.8% | +2.4% | 1.36 | -13.9% | 58% | 14.2 | Y |
| V3 Conservative (Historical) | +100.9% | +26.3% | +2.9% | 1.36 | -15.6% | 57% | 7.2 | Y |
| V3 Standard (Static) | +106.0% | +27.6% | +4.1% | 1.45 | -13.8% | 67% | 9.5 | Y |
...
**前视偏差约 +0.8% Alpha。** 修复后策略仍然有效（Alpha 仍为正）。
...
4. **7年回测覆盖牛熊**: 在 2020 崩盘和 2022 熊市中也表现稳健
...
| 配置 | 年化 | Alpha | 夏普 | 最大回撤 |
|------|------|-------|------|---------|
| V3.1 Standard | +15.7% | -7.7% | 0.97 | -11.8% |
| V3 Standard (对比) | +26.8% | +3.3% | 1.41 | -13.7% |

### V3.1 失败分析

**Alpha 从 +3.3% 暴跌到 -7.7%。** 原因：
...
| 版本 | Alpha | 夏普 | 最大回撤 | 核心改进 |
|------|-------|------|---------|---------|
| V1 | -13.7% | 0.48 | -25.3% | 纯技术/价值策略 |
| V2 | -1.5% | 1.42 | -17.1% | 组合重构 + 动量 + Regime + Trailing Stop |
| V3 | +3.3% | 1.41 | -13.7% | S&P 100 + 历史财报 + 7年回测 |
| V3.1 | -0.1% | 0.97 | -11.8% | 扩到150只（失败，标的池稀释） |
| **V4** | **+3.9%** | **1.44** | **-13.7%** | **跨资产 Regime Filter** |
...
*V3 报告: `reports/portfolio_v3_validation_*.txt`*
*V4 报告: `reports/portfolio_v4_*.txt`*
...
| V2 | -1.5% | 1.42 | -17.1% | 组合重构 + 动量 + Regime + Trailing Stop |
| V3 | +3.3% | 1.41 | -13.7% | S&P 100 + 历史财报 + 7年回测 |
| V3.1 | -0.1% | 0.97 | -11.8% | 扩到150只（失败） |
| V4 | +3.9% | 1.44 | -13.7% | 跨资产 Regime Filter |
| **V5** | **+4.0%** | **1.44** | **-13.7%** | **Short Interest + Insider Trading** |
...
*V5 报告: `reports/portfolio_v5_*.txt`*
...
3. **3年回测区间**: 比 V4/V5 的 7 年窄，统计显著性弱
...
| V3 | +3.3% | 1.41 | -13.7% | S&P 100 + 历史财报 + 7年回测 | 2018-2025 |
| V3.1 | -0.1% | 0.97 | -11.8% | 扩到150只（失败） | 2018-2025 |
| V4 | +3.9% | 1.44 | -13.7% | 跨资产 Regime Filter | 2018-2025 |
| V5 | +4.0% | 1.44 | -13.7% | Short Interest + Insider Trading | 2018-2025 |
| **V6** | **+4.0%*** | **1.44*** | **-13.7%*** | **新闻情绪因子** | 2018-2025* |

\* V6 的 7 年指标沿用 V5（新闻数据仅覆盖至 2020，2021+ 无数据时新闻因子不生效）。在 2018-2020 的 3 年子区间中，V6 比 V5 Alpha 好 +0.77%。
...
*V6 报告: `reports/portfolio_v6_news_*.json`*
...
V1→V6 每一版的"改进"都是看着 2018-2025 回测结果来调的。每加一条规则回测变好了，但自由度也多了一个。
