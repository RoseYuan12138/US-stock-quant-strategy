...
CONFIGS = {
    'V5 Standard': PortfolioConfig(**BASE_CONFIG),
...
def main():
    ...
    # 报告
    print(f"\n\n{'='*90}")
    print(f"V5 策略 — 2025年 回测报告")
    print(f"{'='*90}")

    print(f"\n{'Config':25s} {'Return%':>9s} {'Alpha%':>8s} {'Sharpe':>7s} {'MaxDD%':>7s} {'WinR%':>6s} {'AvgPos':>7s} {'Beat':>5s}")
    print("-" * 80)

    for name, data in all_reports.items():
        r = data['report']
        beat = "Y" if r['beat_benchmark'] else "N"
        print(f"{name:25s} {r['total_return_pct']:+8.1f}% {r['alpha_pct']:+7.1f}% "
    ...
    if all_reports:
        ...
        print(f"{'SPY Buy & Hold':25s} {r['benchmark_return_pct']:+8.1f}% {'0.0':>7s}% "
    ...
    print("月度选股明细")
    print(f"{'='*90}")
    if 'V5 Standard' in all_reports:
        r = all_reports['V5 Standard']['report']
        for rb in r.get('rebalance_log', []):
            date = rb['date']
            regime = rb['regime']
            mult = rb['multiplier']
            stocks = rb['selected'][:10]
            scores = rb.get('scores', {})
            stock_str = ', '.join(f"{s}({scores.get(s, 0):.0f})" for s in stocks)
            print(f"  {date.strftime('%Y-%m-%d')} [{regime:7s} x{mult:.0%}] "
                  f"${rb['total_value']:>10,.0f}  → {stock_str}")

    # 交易明细摘要
    if 'V5 Standard' in all_reports:
        trades = all_reports['V5 Standard']['trade_log']
        sells = [t for t in trades if t['action'] == 'SELL' and t.get('reason') != 'BACKTEST_END']
        if sells:
            print(f"\n{'='*90}")
            print("交易明细（V5 Standard 卖出）")
            print(f"{'='*90}")
            print(f"  {'Date':12s} {'Ticker':8s} {'Reason':18s} {'Profit%':>8s} {'Days':>5s}")
            print(f"  {'-'*55}")
            for t in sells:
                pct = t.get('profit_pct', 0)
                days = t.get('hold_days', 0)
                print(f"  {t['date'].strftime('%Y-%m-%d'):12s} {t['ticker']:8s} "
                      f"{t['reason']:18s} {pct:+7.1f}% {days:5d}")
    ...
    with open(output_dir / f"portfolio_v5_2025only_{ts}.json", 'w') as f:
        json.dump(json_data, f, indent=2, default=str)
    print(f"\nJSON 已保存: reports/portfolio_v5_2025only_{ts}.json")
