...
V4 组合策略验证
改进点（相对V3）：
1. 跨资产 Regime Filter：收益率曲线 + VIX + 信用利差 + SPY 均线
2. 对比 V3（仅SPY均线）vs V4（跨资产信号），同一标的池和参数

预期效果：MaxDD 改善 3-5%，Alpha 持平或小幅提升
"""
...
CONFIGS = {
    'V4 Cross-Asset Regime': PortfolioConfig(**BASE_CONFIG),
    'V3 SMA-Only Regime (baseline)': PortfolioConfig(**BASE_CONFIG),
    'V4 Aggressive': PortfolioConfig(
        **{**BASE_CONFIG,
    ...
    'V4 Conservative': PortfolioConfig(
        **{**BASE_CONFIG,
...
def run_validation():
    ...
    print(f"新增: 跨资产 Regime Filter (收益率曲线 + VIX + 信用利差)")
    print(f"{'='*80}")

    # 1. 宏观数据
    print(f"\n[Step 1] 获取宏观数据...")
    macro_data = fetch_macro_data(price_fetcher)
    macro_available = len(macro_data) >= 3  # 至少有3个信号
    if not macro_available:
        print(f"  WARNING: 宏观数据不足（{len(macro_data)}/5），跨资产信号将降级")
    ...
    for config_name, config in CONFIGS.items():
        ...
        # V3 baseline 不传 macro_data（退化为仅 SPY 均线）
        use_macro = 'V3' not in config_name
        current_macro = macro_data if use_macro else None
    ...
    return all_reports, macro_data


def generate_report(all_reports, macro_data):
    lines = []
    lines.append("=" * 95)
    lines.append("V4 组合策略验证报告")
    lines.append(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    lines.append(f"周期: {START_DATE} ~ {END_DATE}")
    lines.append(f"标的池: S&P 100 ~{len(SP100_TICKERS)} 只")
    lines.append(f"改进: 跨资产 Regime Filter (收益率曲线 + VIX + 信用利差)")
    lines.append(f"宏观数据: {', '.join(k for k in macro_data.keys())}")
    lines.append("=" * 95)

    # 对比表
    lines.append(f"\n{'Config':35s} {'Return%':>9s} {'Annual%':>8s} {'Alpha%':>8s} "
    ...
    for name, data in all_reports.items():
        ...
        lines.append(
            f"{name:35s} {r['total_return_pct']:+8.1f}% {r['annual_return_pct']:+7.1f}% "
    ...
    if all_reports:
        ...
        lines.append(
            f"{'SPY Buy & Hold':35s} {r['benchmark_return_pct']:+8.1f}% "
            f"{r['benchmark_annual_pct']:+7.1f}% {'0.0':>7s}% "
            f"{r['benchmark_sharpe']:7.2f} {r['benchmark_max_dd_pct']:6.1f}%"
        )

    # V3 vs V4 对比
    v4_key = 'V4 Cross-Asset Regime'
    v3_key = 'V3 SMA-Only Regime (baseline)'
    if v4_key in all_reports and v3_key in all_reports:
        r4 = all_reports[v4_key]['report']
        r3 = all_reports[v3_key]['report']
        lines.append(f"\n{'='*95}")
        lines.append("V3 vs V4 Regime Filter 对比")
        lines.append(f"{'='*95}")
        lines.append(f"  {'指标':20s} {'V4 (跨资产)':>14s} {'V3 (仅SMA)':>14s} {'差值':>12s}")
        lines.append(f"  {'-'*65}")
        for key, label in [('total_return_pct', '总收益'),
            ...
            lines.append(f"  {label:20s} {v4:>13{fmt}}{unit} {v3:>13{fmt}}{unit} {diff:>+11{fmt}}{unit}")

        # 关键改善
        dd_improvement = r4['max_drawdown_pct'] - r3['max_drawdown_pct']
        if dd_improvement > 0:
            lines.append(f"\n  回撤改善: {dd_improvement:+.1f}% (V4 回撤更小)")
        alpha_diff = r4['alpha_pct'] - r3['alpha_pct']
        lines.append(f"  Alpha 变化: {alpha_diff:+.2f}%")

        # Regime 分布对比
        lines.append(f"\n  市场环境分布:")
        lines.append(f"    V4: {r4.get('regime_days', {})}")
        lines.append(f"    V3: {r3.get('regime_days', {})}")
    ...
    if v4_key in all_reports and v3_key in all_reports:
        r4 = all_reports[v4_key]['report']
        r3 = all_reports[v3_key]['report']
        dd_better = r4['max_drawdown_pct'] > r3['max_drawdown_pct']
        alpha_better = r4['alpha_pct'] > r3['alpha_pct']

        if dd_better and alpha_better:
            lines.append("结论: V4 全面优于 V3（Alpha更高 + 回撤更小）")
        elif dd_better:
            lines.append("结论: V4 回撤控制更优，Alpha 略有变化")
        elif alpha_better:
            lines.append("结论: V4 Alpha 更高，但回撤控制无明显改善")
        else:
            lines.append("结论: V4 跨资产信号在本回测周期内未显著改善")
...
def main():
    all_reports, macro_data = run_validation()
    if not all_reports:
        print("\n没有结果")
        return

    report_text = generate_report(all_reports, macro_data)
    print("\n\n" + report_text)
    ...
    with open(output_dir / f"portfolio_v4_{ts}.txt", 'w') as f:
        f.write(report_text)

    json_data = {n: {k: v for k, v in d['report'].items() if k != 'rebalance_log'}
                 for n, d in all_reports.items()}
    with open(output_dir / f"portfolio_v4_{ts}.json", 'w') as f:
        json.dump(json_data, f, indent=2, default=str)

    print(f"\n报告已保存到 reports/portfolio_v4_{ts}.*")
