...
> ## 📋 数据修正记录（2026-03-30）
>
> **以下错误已于 2026-03-30 完成修正，本文档当前版本已反映正确数据：**
>
> - **回测区间标注**：V3/V3.1/V4/V5 原标注为 `2018–2025`（7年）有误，实际均为 `2023–2025`（3年）。已全部修正。
> - **V3 数值修正**：Static 模式总收益从 +106.0% 改为 +116.9%（Alpha +4.1% → +6.1%）；前视偏差从 0.8% 修正为 2.8%。
> - **V3.1 数值修正**：Alpha 从 -7.7% 改为 -0.1%，夏普从 0.97 改为 1.19，并补全完整配置对比表。
> - **V6 汇总修正**：§23 汇总表中 V6 Alpha 从沿用 V5 的 `+4.0%*` 改为实际值 `-3.6%`（Standard，2018-2020）。
> - **V3 长周期验证**：已于 2026-03-30 在 2018–2025（8年）区间重跑，结果见 §14 末尾注及 `reports/portfolio_v3_validation_20260330_1540.json`。
>
> **经核实的 V3 基准数据（yfinance）见 `baseline_metrics.md`，FMP 数据源对比见 `fmp_vs_yfinance_comparison.md`。**
...
| **V6** | 2026-03-29 | 历史新闻情绪因子（HuggingFace 数据集） | +1.3% | 2018-2020（3年）² |

> ¹ V3 于 2026-03-30 在 2018-2025（8年）区间重跑，见 `reports/portfolio_v3_validation_20260330_1540.json`，结果见 §14 末尾注。
> ² V6 最佳为 Aggressive 配置，Alpha +1.3%（2018-2020）。Standard 配置 Alpha -3.6%。V6 区间与 V1-V5 不同（含 2020 COVID 崩盘），不可直接比较。
